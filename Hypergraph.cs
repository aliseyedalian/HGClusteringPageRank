...
namespace HGPRC
{
    public class Hypergraph
    {
        ...
        public List<List<int>> incident_edges = new();
        public List<double> weights = new();
        public Dictionary<List<int>, int> ID = new();
        public Dictionary<int, List<int>> ID_rev = new();
        public Dictionary<int , string> node_name = new();
        public Dictionary<string, int> node_id = new(); 
        ...
        public static Hypergraph Open(string fn)
        {
            ...
            int vertex_num = 0;
            Dictionary<string,int> node_id = new();
            Dictionary<int,string> node_name = new();

            for (string? line; (line = sr.ReadLine()) != null;)
            {
                var words = line.Split();
                double weight = double.Parse(words.Last());
                if (weight != 0)
                {
                    var edge = new List<int>();
                    int i = 0;
    ...
    } // End of public class Hypergraph


    public class Graph
    {
        public List<Dictionary<int, double>> adj_list = new();
